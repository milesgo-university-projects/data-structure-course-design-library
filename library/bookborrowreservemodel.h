...
******************************************************************/
#ifndef BOOKBORROWRESERVEMODEL
#define BOOKBORROWRESERVEMODEL
...
class bookBorrowOrReserveTableModel:public QAbstractTableModel
{
    Q_OBJECT
public:
    bookBorrowOrReserveTableModel(QWidget *parent=0);
    ~bookBorrowOrReserveTableModel();
    int rowCount(const QModelIndex &parent) const;//设置表格有几行
    int columnCount(const QModelIndex &parent) const;//设置表格有几列
    QVariant data(const QModelIndex &index, int role) const;
    void updateData(QList<class BorrowOrReserve> list);//更新model中的数据
    ...
    QList<class BorrowOrReserve> pList;//存储书的容器
    QStringList headerName;//存储表头的容器
    int horizontalHeaderCount;
};

#endif // BOOKBORROWRESERVEMODEL
